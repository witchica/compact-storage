...
public class CompactChestBlockEntityRenderer implements BlockEntityRenderer<CompactChestBlockEntity> {
    ...
    static {
        for(int i = 0; i < 16; i++) {
            CHEST_TEXTURES.put(CompactStorage.COMPACT_CHEST_BLOCKS[i].get(), new Identifier("compact_storage", String.format("textures/block/compact_chest_%s.png", DyeColor.byId(i).name().toLowerCase())));
        }
    }
    ...
    public void render(CompactChestBlockEntity compactChestBlockEntity, float delta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int light, int overlay) {
        ...
        BlockState blockState = bl ? compactChestBlockEntity.getCachedState() : CompactStorage.COMPACT_CHEST_BLOCKS[0].get().getDefaultState().with(CompactChestBlock.FACING, Direction.SOUTH);
